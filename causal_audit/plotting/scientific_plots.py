...
These complement core_plots_v01.py for comprehensive paper figures.
"""
...
def plot_stationarity_diagnostics(
    ...
    for i, var in enumerate(variables):
        ...
        for val, metric in zip(values, metrics):
            if 'Break' in metric:
                colors.append(PASTEL['poor'] if val > 2.0 else PASTEL['moderate'] if val > 1.0 else PASTEL['excellent'])
            else:
                colors.append(PASTEL['poor'] if val > 0.01 else PASTEL['moderate'] if val > 0.005 else PASTEL['excellent'])
...
def plot_persistence_analysis(
    ...
    ax1 = axes[0]
    colors1 = [PASTEL['poor'] if tau > 50 else PASTEL['moderate'] if tau > 10 else PASTEL['excellent'] for tau in integral_taus]
    bars1 = ax1.bar(variables, integral_taus, color=colors1, alpha=0.8, edgecolor='gray')
    ax1.set_ylabel('Integral Autocorrelation Time (τ)')
    ax1.set_title('Temporal Persistence')
    ax1.axhline(y=10, color='orange', linestyle='--', alpha=0.7, label='Moderate (τ=10)')
    ...
    ax2 = axes[1]
    colors2 = [PASTEL['poor'] if eff < 30 else PASTEL['moderate'] if eff < 100 else PASTEL['excellent'] for eff in t_effs]
    ...
    ax2.axhline(y=30, color='red', linestyle='--', alpha=0.7, label='Critical (N_eff=30)')
    ax2.axhline(y=100, color='orange', linestyle='--', alpha=0.7, label='Adequate (N_eff=100)')
    ax2.legend()
    ...
    ax3 = axes[2]
    colors3 = [PASTEL['poor'] if ratio < 0.1 else PASTEL['moderate'] if ratio < 0.2 else PASTEL['excellent'] for ratio in t_eff_ratios]
...
def plot_method_suitability_matrix(
    ...
    if method_catalog is None:
        method_catalog = {
            "PCMCI+": {"stationarity": 0.8, "irregularity": 0.9, "persistence": 0.7, "confounding": 0.6, "nonlinearity": 0.4},
            "Granger": {"stationarity": 0.6, "irregularity": 0.5, "persistence": 0.8, "confounding": 0.4, "nonlinearity": 0.2},
            "VAR-LiNGAM": {"stationarity": 0.7, "irregularity": 0.6, "persistence": 0.6, "confounding": 0.8, "nonlinearity": 0.3},
            "CD-NOD": {"stationarity": 0.9, "irregularity": 0.7, "persistence": 0.5, "confounding": 0.5, "nonlinearity": 0.6},
            "Transfer Entropy": {"stationarity": 0.5, "irregularity": 0.8, "persistence": 0.9, "confounding": 0.3, "nonlinearity": 0.9}
        }
    ...
    risk_names = ["stationarity", "irregularity", "persistence", "confounding", "nonlinearity"]
    current_risks = [risks[risk_mapping[name]]["mean"] if name in risk_mapping else 0.3 for name in risk_names]
    ...
    for j, risk_val in enumerate(current_risks):
        ax.text(j, -0.9, f'Current Data:\n{risk_val:.2f}', ha='center', va='top', 
                fontsize=8, color='red', fontweight='bold')
    
    ax.set_title('Method Suitability Matrix\n(Based on Current Data Risk Profile)', 
...
def plot_calibration_curve(
    ...
    observed_freqs = predicted_probs + np.random.normal(0, 0.05, 10)  # Near-perfect calibration
    ...
    ax1.scatter(predicted_probs, observed_freqs, s=n_samples, alpha=0.7, 
    ...
    ax1.errorbar(predicted_probs, observed_freqs, yerr=errors, fmt='none', 
    ...
    slope, intercept, r_value, p_value, std_err = stats.linregress(predicted_probs, observed_freqs)
...
def plot_temporal_aggregation_analysis(
    ...
    if half_hourly_data is None or daily_data is None:
        ...
        half_hourly_data = pd.DataFrame({'value': half_hourly_values}, index=t_half_hourly)
    ...
    ax1 = axes[0, 0]
    ax1.plot(half_hourly_data.index[:48*7], half_hourly_data['value'][:48*7], 
             'b-', alpha=0.7, linewidth=0.5, label='Half-hourly')
    ax1.plot(daily_data.index[:7], daily_data['value'][:7], 
             'ro-', markersize=6, linewidth=2, label='Daily (aggregated)')
    ax1.set_xlabel('Time')
    ...
    ax2.plot(lags_half_hourly, half_hourly_autocorr, 'b-', linewidth=2, 
             label=f'Half-hourly (τ={tau_half_hourly:.1f})')
    ax2.plot(lags_daily, daily_autocorr, 'r-', linewidth=2, 
    ...
    f_half, psd_half = signal.periodogram(half_hourly_data['value'], fs=48)  # 48 samples per day
    f_daily, psd_daily = signal.periodogram(daily_data['value'], fs=1)  # 1 sample per day
    ...
    bars1 = ax4.bar(x - width/2, raw_samples, width, label='Raw Samples', alpha=0.7, color='lightblue')
    bars2 = ax4.bar(x + width/2, eff_samples, width, label='Effective Samples', alpha=0.7, color='orange')
    ...
    fig.suptitle(f'FLUXNET-CH4 Temporal Aggregation Analysis\n'
...
def plot_validation_performance(
    ...
    """
    benchmarks = ['Synthetic Atlas\n(500 DGPs)', 'TimeGraph Benchmark\n(18 categories)', 
                  'CausalTime Benchmark\n(3 scenarios)', 'FLUXNET-CH4 Dataset\n(5 wetland sites)']
    ...
    ax1 = axes[0, 0]
    colors1 = [PASTEL['excellent'] if score >= 0.95 else 
               PASTEL['good'] if score >= 0.9 else 
               PASTEL['moderate'] for score in auroc_scores]
    bars1 = ax1.bar(benchmarks, auroc_scores, color=colors1, alpha=0.8, edgecolor='gray')
    ax1.axhline(y=0.8, color='red', linestyle='--', alpha=0.7, linewidth=2, label='Target (0.80)')
    ...
    for bar, val in zip(bars1, auroc_scores):
        height = bar.get_height()
        performance = "Excellent" if val >= 0.95 else "Very Good" if val >= 0.9 else "Good"
    ...
    bars2a = ax2.bar(x - width/2, precision_scores, width, label='Precision', 
                     alpha=0.8, color=PASTEL['tertiary'], edgecolor='gray')
    bars2b = ax2.bar(x + width/2, recall_scores, width, label='Recall', 
                     alpha=0.8, color=PASTEL['accent1'], edgecolor='gray')
    
    ax2.axhline(y=0.9, color='red', linestyle='--', alpha=0.7, linewidth=2, label='Precision Target')
    ax2.axhline(y=0.85, color='orange', linestyle='--', alpha=0.7, linewidth=2, label='Recall Target')
    ...
    ax3 = axes[1, 0]
    bars3 = ax3.bar(benchmarks, pass_rates, color=PASTEL['primary'], 
    ...
    for bar, val in zip(bars3, pass_rates):
        height = bar.get_height()
        ax3.annotate(f'{val:.0%}\n(Perfect)', xy=(bar.get_x() + bar.get_width()/2, height),
                    xytext=(0, 5), textcoords="offset points",
                    ha='center', va='bottom', fontsize=9, fontweight='bold')
    ...
    colors = [PASTEL['primary'], PASTEL['tertiary'], PASTEL['accent1'], PASTEL['secondary']]
    
    for i, (benchmark, color) in enumerate(zip(benchmark_names, colors)):
        offset = (i - 1.5) * bar_width
        bars = ax4.bar(x_pos + offset, data_matrix[i], bar_width, 
                      label=benchmark, color=color, alpha=0.8, edgecolor='gray')
    ...
    for i, target in enumerate(targets):
        ax4.axhline(y=target, xmin=i/len(targets), xmax=(i+1)/len(targets), 
    ...
    ax4.text(0.98, 0.02, f'Average Performance:\nAUROC: {overall_scores[0]:.3f}\nPrecision: {overall_scores[1]:.3f}\nRecall: {overall_scores[2]:.3f}\nPass Rate: {overall_scores[3]:.3f}',
             transform=ax4.transAxes, fontsize=9, fontweight='bold', ha='right', va='bottom',
...
def plot_nonlinearity_diagnostics(
    ...
    bars1 = ax1.bar(x - width/2, linear_rmse, width, label='Linear AR', 
                    alpha=0.8, color=PASTEL['good'], edgecolor='gray')
    bars2 = ax1.bar(x + width/2, rf_rmse, width, label='Random Forest', 
                    alpha=0.8, color=PASTEL['moderate'], edgecolor='gray')
    ...
    colors = [PASTEL['excellent'] if delta < -2 else PASTEL['moderate'] if delta < -1 else PASTEL['poor'] for delta in delta_rmse]
    bars = ax2.bar(variables, [abs(d) for d in delta_rmse], color=colors, alpha=0.8, edgecolor='gray')
...
def plot_real_world_walkthrough(
    output_path: Optional[str] = None
) -> str:
    """
    ...
    ax1.axvline(dates[break_point], color='red', linestyle='--', alpha=0.7, label='Structural Break')
    ax1.set_title('A. Synthetic Example: Raw Time Series (Problematic)', fontweight='bold')
    ax1.set_ylabel('Value')
    ax1.legend()
    ax1.grid(alpha=0.3)
    
    # Add problem annotations pointing to correct features
    ax1.annotate('Strong Trend', xy=(dates[n//4], x_gapped[n//4]), xytext=(dates[n//4], x_gapped[n//4]+2),
    ...
    if missing_idx > 0 and missing_idx < len(dates)-1:
        ax1.annotate('Missing Data', xy=(dates[missing_idx], y_gapped[missing_idx-1]), 
    ...
    diagnostics = ['Stationarity', 'Irregularity', 'Persistence', 'Nonlinearity', 'Confounding']
    risk_levels = [0.85, 0.65, 0.45, 0.35, 0.75]  # High risks
    colors = [PASTEL['poor'] if r > 0.7 else PASTEL['moderate'] if r > 0.3 else PASTEL['excellent'] for r in risk_levels]
    ...
    for bar, risk in zip(bars, risk_levels):
        ...
        ax2.annotate(f'{risk:.2f} ({level})', xy=(width, bar.get_y() + bar.get_height()/2),
    ...
    bars1 = ax3.bar(x_pos - width/2, false_positives, width, label='False Positive Rate',
                   color=PASTEL['poor'], alpha=0.8, edgecolor='gray')
    bars2 = ax3.bar(x_pos + width/2, precision, width, label='Precision',
                   color=PASTEL['excellent'], alpha=0.8, edgecolor='gray')
    ...
    bars = ax5.bar(methods_comp, edge_counts, color=colors_comp, alpha=0.8, edgecolor='gray')
    ...
    for bar, count in zip(bars, edge_counts):
        height = bar.get_height()
        ax5.annotate(f'{count}', xy=(bar.get_x() + bar.get_width()/2, height),
                    xytext=(0, 3), textcoords='offset points',
                    ha='center', va='bottom', fontsize=11, fontweight='bold')
    
    # Final summary
    ax6 = axes[2, 1]
    ax6.text(0.5, 0.8, 'OUTCOME SUMMARY', ha='center', va='center', 
             fontsize=14, fontweight='bold', fontfamily=PUBLICATION_FONT)
...
def plot_comprehensive_calibration(
    ...
    for i, (risk_type, color) in enumerate(zip(risk_types, colors)):
        ...
        ax1.plot(predicted, observed, 'o-', color=color, label=risk_type, 
    ...
    regimes = ['Short Series\n(n<100)', 'Medium Series\n(100≤n<500)', 'Long Series\n(n≥500)']
    ...
    bars1 = ax2.bar(x_pos - width/2, auroc_scores, width, label='AUROC',
                   color=PASTEL['excellent'], alpha=0.8, edgecolor='gray')
    
    ax2_twin = ax2.twinx()
    bars2 = ax2_twin.bar(x_pos + width/2, brier_scores, width, label='Brier Score',
    ...
    ax2_twin.axhline(y=0.2, color='red', linestyle='--', alpha=0.7, label='Brier Target')
    ...
    conf_intervals = 1.96 * np.sqrt(observed_freqs * (1 - observed_freqs) / sample_sizes)
    
    ax3.errorbar(bin_centers, observed_freqs, yerr=conf_intervals, 
                fmt='o', capsize=5, capthick=2, color=PASTEL['primary'], 
                ecolor='gray', alpha=0.8, linewidth=2, markersize=8)
    ...
    bars = ax4.bar(methods, auroc_comparison, color=colors_comp, alpha=0.8, edgecolor='gray')
    ...
    for bar, score in zip(bars, auroc_comparison):
        height = bar.get_height()
        ax4.annotate(f'{score:.2f}', xy=(bar.get_x() + bar.get_width()/2, height),
                    xytext=(0, 3), textcoords='offset points',
                    ha='center', va='bottom', fontsize=11, fontweight='bold')
    ...
    for i in range(len(risk_names)):
        for j in range(len(metrics)):
            text = ax5.text(j, i, f'{calib_matrix[i, j]:.2f}',
                           ha="center", va="center", color="black", fontweight='bold')
    ...
    ax6.semilogx(sample_sizes, uncertainty_widths, 'o-', color=PASTEL['primary'], 
    ...
    for size, width in zip(sample_sizes[::2], uncertainty_widths[::2]):
        ax6.annotate(f'{width:.2f}', xy=(size, width), xytext=(0, 10),
...
def plot_method_failure_analysis(
    output_path: Optional[str] = None
) -> str:
    """
    ...
    ax1.contourf(X, Y, pcmci_boundary.astype(int), levels=[0, 0.5, 1], 
                colors=[PASTEL['poor'], PASTEL['excellent']], alpha=0.3)
    ax1.contour(X, Y, pcmci_boundary.astype(int), levels=[0.5], 
               colors=['blue'], linewidths=2, linestyles='-')
    ax1.contour(X, Y, granger_boundary.astype(int), levels=[0.5], 
               colors=['red'], linewidths=2, linestyles='--')
    ax1.contour(X, Y, var_boundary.astype(int), levels=[0.5], 
    ...
    bars1 = ax2.bar(x_pos - width/2, success_rates, width, label='Success Rate',
                   color=PASTEL['excellent'], alpha=0.8, edgecolor='gray')
    bars2 = ax2.bar(x_pos + width/2, failure_rates, width, label='Failure Rate',
                   color=PASTEL['poor'], alpha=0.8, edgecolor='gray')
    ...
    ax3.plot(thresholds, precision, 'o-', color=PASTEL['excellent'], 
            linewidth=2, markersize=6, label='Precision')
    ax3.plot(thresholds, recall, 's-', color=PASTEL['moderate'], 
    ...
    ax3.axvline(optimal_threshold, color='red', linestyle='--', alpha=0.7, 
    ...
    bars1 = ax4.bar(x_pos - width/2, type_i_errors, width, label='Type I Error\n(False Positive)',
                   color=PASTEL['moderate'], alpha=0.8, edgecolor='gray')
    bars2 = ax4.bar(x_pos + width/2, type_ii_errors, width, label='Type II Error\n(False Negative)',
                   color=PASTEL['poor'], alpha=0.8, edgecolor='gray')
    ...
    total_costs = [t1 * 1 + t2 * 3 for t1, t2 in zip(type_i_errors, type_ii_errors)]  # Type II more costly
    for i, (bar1, bar2, cost) in enumerate(zip(bars1, bars2, total_costs)):
        ax4.text(i, max(type_i_errors[i], type_ii_errors[i]) + 0.05, 
                f'Total Cost: {cost:.2f}', ha='center', fontweight='bold')
    ...
    colors_roc = [PASTEL['excellent'], PASTEL['moderate'], PASTEL['good'], PASTEL['poor']]
    ...
    for method, color in zip(methods_roc, colors_roc):
        ...
        if method != 'Random':  # Don't duplicate random line
            ax5.plot(fpr_base, tpr, 'o-', color=color, linewidth=2, 
                    markersize=4, label=method, alpha=0.8)
    ...
    decision_types = ['Correct\nRecommendation', 'Wrong\nRecommendation', 'Missed\nProblem', 'Correct\nAbstention']
    frequencies = [0.65, 0.10, 0.15, 0.10]
    costs = [0, 5, 10, 1]  # Relative costs
    total_costs = [f * c for f, c in zip(frequencies, costs)]
    
    colors_cost = [PASTEL['excellent'], PASTEL['moderate'], PASTEL['poor'], PASTEL['good']]
    
    bars = ax6.bar(decision_types, total_costs, color=colors_cost, alpha=0.8, edgecolor='gray')
...
def plot_computational_cost_analysis(
    output_path: Optional[str] = None
) -> str:
    """
    ...
    ax1.loglog(sample_sizes, causal_audit_time, 'o-', color=PASTEL['excellent'], 
              linewidth=2, markersize=6, label='causal-audit')
    ax1.loglog(sample_sizes, pcmci_time, 's-', color=PASTEL['moderate'], 
              linewidth=2, markersize=6, label='PCMCI+')
    ax1.loglog(sample_sizes, granger_time, '^-', color=PASTEL['good'], 
              linewidth=2, markersize=6, label='Granger')
    ...
    bars1 = ax2.bar(x_pos - width/2, audit_costs, width, label='Audit Cost (min)',
                   color=PASTEL['moderate'], alpha=0.8, edgecolor='gray')
    
    ax2_twin = ax2.twinx()
    bars2 = ax2_twin.bar(x_pos + width/2, error_costs, width, label='Error Cost (hours)',
                        color=PASTEL['poor'], alpha=0.8, edgecolor='gray')
    ...
    # Add ROI annotations
    roi_values = [(error_costs[0] - error_costs[i]) * 60 / max(audit_costs[i], 1) - 1 
                  for i in range(len(scenarios))]
    for i, roi in enumerate(roi_values[1:], 1):  # Skip baseline
        ax2.text(i, audit_costs[i] + 2, f'ROI: {roi:.1f}x', 
    ...
    colors_diag = [PASTEL['excellent'], PASTEL['good'], PASTEL['moderate'], 
                   PASTEL['poor'], PASTEL['accent1']]
    
    wedges, texts, autotexts = ax3.pie(runtimes, labels=diagnostics, colors=colors_diag,
                                      autopct='%1.1f%%', startangle=90)
    # Set alpha for wedges
    for wedge in wedges:
        wedge.set_alpha(0.8)
    ax3.set_title('C. Diagnostic Runtime Breakdown\n(n=1000 samples)', fontweight='bold')
    ...
    bars = ax4.bar(approaches, efficiency, color=[PASTEL['poor'], PASTEL['moderate'], PASTEL['excellent']], 
                  alpha=0.8, edgecolor='gray')
...
def plot_framework_architecture(
    ...
    modules = {
        'Module A': {'pos': (0.1, 0.7), 'size': (0.25, 0.25), 'color': PASTEL['excellent']},
        'Module B': {'pos': (0.4, 0.7), 'size': (0.25, 0.25), 'color': PASTEL['moderate']},
        'Module C': {'pos': (0.7, 0.7), 'size': (0.25, 0.25), 'color': PASTEL['primary']}
    }
    
    # Draw modules
    for name, props in modules.items():
        rect = Rectangle(props['pos'], props['size'][0], props['size'][1], 
                        facecolor=props['color'], alpha=0.7, edgecolor='black', linewidth=2)
        ax.add_patch(rect)
        
        # Add module labels
        center_x = props['pos'][0] + props['size'][0]/2
        center_y = props['pos'][1] + props['size'][1]/2
        ax.text(center_x, center_y + 0.08, name, ha='center', va='center', 
               fontsize=14, fontweight='bold')
        
        if name == 'Module A':
            ax.text(center_x, center_y, 'Assumption\nAuditor', ha='center', va='center', 
                   fontsize=12, fontweight='bold')
            ax.text(center_x, center_y - 0.08, '5 Diagnostics\n• Stationarity\n• Irregularity\n• Persistence\n• Nonlinearity\n• Confounding', 
                   ha='center', va='center', fontsize=9)
        elif name == 'Module B':
            ax.text(center_x, center_y, 'Uncertainty\nQuantifier', ha='center', va='center', 
                   fontsize=12, fontweight='bold')
            ax.text(center_x, center_y - 0.08, '4 Risk Indices\n• Calibrated models\n• Credible intervals\n• Risk ledgers\n• T_eff adjustment', 
                   ha='center', va='center', fontsize=9)
        else:  # Module C
            ax.text(center_x, center_y, 'Bayesian\nRecommender', ha='center', va='center', 
                   fontsize=12, fontweight='bold')
            ax.text(center_x, center_y - 0.08, 'Decision Support\n• Utility maximization\n• Method selection\n• Principled abstention\n• Transparent rationale', 
                   ha='center', va='center', fontsize=9)
    ...
    ax.text(0.95, 0.5, 'OUTPUT:\nMethod\nRecommendation\n+ Risk Profile', ha='center', va='center', 
           fontsize=12, fontweight='bold',
           bbox=dict(boxstyle='round', facecolor=PASTEL['accent1'], alpha=0.8))
...
3. Principled Decision Support
   ...
   • Transparent abstention criteria'''
    ...
    ax.text(0.5, 0.35, innovations_text, ha='center', va='top', fontsize=11,
           bbox=dict(boxstyle='round', facecolor=PASTEL['accent2'], alpha=0.9))
    
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ax.axis('off')
    ax.set_title('causal-audit Framework Architecture and Scientific Contributions', 
...
def plot_benchmark_comparison(
    ...
    bars1 = ax1.bar(x - width, systematic, width, label='Systematic Assessment', 
                   color=PASTEL['excellent'], alpha=0.8)
    bars2 = ax1.bar(x, quantitative, width, label='Quantitative Risk', 
                   color=PASTEL['moderate'], alpha=0.8)
    bars3 = ax1.bar(x + width, automated, width, label='Automation Level', 
                   color=PASTEL['good'], alpha=0.8)
    ...
    bars1 = ax2.bar(x - width/2, causal_audit_perf, width, label='causal-audit', 
                   color=PASTEL['excellent'], alpha=0.8)
    bars2 = ax2.bar(x + width/2, baseline_perf, width, label='Baseline Methods', 
                   color=PASTEL['poor'], alpha=0.8)
    ...
    ax3.loglog(sample_sizes, causal_audit_time, 'o-', color=PASTEL['excellent'], 
              linewidth=3, markersize=8, label='causal-audit (pre-analysis)')
    ax3.loglog(sample_sizes, full_analysis_time, 's-', color=PASTEL['moderate'], 
              linewidth=3, markersize=8, label='Full causal discovery')
    ...
    bars1 = ax4.bar(x - width/2, before_scores, width, label='Before causal-audit', 
                   color=PASTEL['poor'], alpha=0.8)
    bars2 = ax4.bar(x + width/2, after_scores, width, label='With causal-audit', 
                   color=PASTEL['excellent'], alpha=0.8)
    ...
    for i, improvement in enumerate(improvements):
        ax4.text(i, max(before_scores[i], after_scores[i]) + 0.05, 
                f'+{improvement:.0f}%', ha='center', fontweight='bold', color='green')
...
def plot_dataset_dashboards(
    ...
    colors1 = [PASTEL['excellent'], PASTEL['good'], PASTEL['moderate'], PASTEL['poor'], PASTEL['critical']]
    
    bars1 = ax1.bar(dgp_types, dgp_counts, color=colors1, alpha=0.8, edgecolor='gray')
    ax1.set_title('A. Synthetic Atlas (500 DGPs)', fontweight='bold', fontfamily=PUBLICATION_FONT)
    ...
    bars2 = ax2.bar(timegraph_cats, timegraph_counts, color=PASTEL['primary'], alpha=0.8, edgecolor='gray')
    ax2.set_title('B. TimeGraph Benchmark (18 categories)', fontweight='bold', fontfamily=PUBLICATION_FONT)
    ...
    ax3 = axes[1, 0]
    causaltime_scenarios = ['Scenario 1\n(Linear)', 'Scenario 2\n(Nonlinear)', 'Scenario 3\n(Mixed)']
    scenario_complexity = [0.3, 0.7, 0.9]
    colors3 = [PASTEL['excellent'], PASTEL['moderate'], PASTEL['poor']]
    
    bars3 = ax3.bar(causaltime_scenarios, scenario_complexity, color=colors3, alpha=0.8, edgecolor='gray')
    ax3.set_title('C. CausalTime Benchmark (3 scenarios)', fontweight='bold', fontfamily=PUBLICATION_FONT)
    ...
    for bar, complexity in zip(bars3, scenario_complexity):
        height = bar.get_height()
        ax3.annotate(f'{complexity:.1f}', xy=(bar.get_x() + bar.get_width()/2, height),
                    xytext=(0, 3), textcoords='offset points',
                    ha='center', va='bottom', fontweight='bold')
    ...
    colors4 = [PASTEL['poor'] if x > 0.7 else PASTEL['moderate'] if x > 0.5 else PASTEL['excellent'] for x in site_characteristics]
    
    bars4 = ax4.bar(fluxnet_sites, site_characteristics, color=colors4, alpha=0.8, edgecolor='gray')
    ax4.set_title('D. FLUXNET-CH4 Dataset (5 wetland sites)', fontweight='bold', fontfamily=PUBLICATION_FONT)
    ...
    for bar, challenge in zip(bars4, site_characteristics):
        ...
        ax4.annotate(f'{challenge:.1f}\n({level})', xy=(bar.get_x() + bar.get_width()/2, height),
                    xytext=(0, 5), textcoords='offset points',
                    ha='center', va='bottom', fontweight='bold')
...
def plot_dgp_atlas_comprehensive(
    output_path: Optional[str] = None
) -> str:
    """
    ...
    families = ['Linear AR', 'VAR', 'Nonlinear AR', 'Regime Switch', 'Structural Break', 'Mixed Effects']
    family_counts = [100, 80, 75, 70, 90, 85]
    colors_fam = [PASTEL['excellent'], PASTEL['good'], PASTEL['moderate'], PASTEL['poor'], PASTEL['critical'], PASTEL['primary']]
    
    bars1 = ax1.bar(families, family_counts, color=colors_fam, alpha=0.8, edgecolor='gray')
    ax1.set_title('A. DGP Family Distribution (500 Total)', fontweight='bold', fontfamily=PUBLICATION_FONT)
    ax1.set_ylabel('Number of DGPs')
    ...
    scatter = ax2.scatter(n_samples, persistence_levels, c=persistence_levels, 
                         cmap='RdYlGn_r', alpha=0.6, s=30, edgecolors='gray', linewidth=0.5)
    ax2.set_xlabel('Sample Size')
    ax2.set_ylabel('Persistence Level')
    ax2.set_title('B. Parameter Space Coverage', fontweight='bold', fontfamily=PUBLICATION_FONT)
    ax2.grid(alpha=0.3)
    ...
    ax3.set_title('C. Stationarity vs Nonlinearity Distribution', fontweight='bold', fontfamily=PUBLICATION_FONT)
    ...
    ax4.hist(autocorr_times, bins=30, color=PASTEL['moderate'], alpha=0.8, edgecolor='gray')
    ax4.axvline(np.median(autocorr_times), color='red', linestyle='--', linewidth=2, label=f'Median: {np.median(autocorr_times):.1f}')
    ax4.set_xlabel('Integral Autocorrelation Time')
    ax4.set_ylabel('Number of DGPs')
    ax4.set_title('D. Temporal Persistence Distribution', fontweight='bold', fontfamily=PUBLICATION_FONT)
    ax4.legend()
    ax4.grid(axis='y', alpha=0.3)
    ...
    examples = {
        'Linear Stationary': np.cumsum(np.random.normal(0, 1, 200)) * 0.1 + np.random.normal(0, 0.5, 200),
        'Nonlinear': np.sin(t/20) + 0.5 * np.sin(t/5) + np.random.normal(0, 0.3, 200),
        'Regime Switch': np.concatenate([np.random.normal(0, 1, 100), np.random.normal(3, 1.5, 100)]),
        'Structural Break': np.concatenate([0.02*t[:100] + np.random.normal(0, 0.5, 100), 
                                          0.08*t[100:] - 8 + np.random.normal(0, 0.8, 100)]),
        'High Persistence': np.cumsum(np.random.normal(0, 0.3, 200))
    }
    
    colors_ex = [PASTEL['excellent'], PASTEL['moderate'], PASTEL['poor'], PASTEL['critical'], PASTEL['primary']]
    
    for i, (name, series) in enumerate(examples.items()):
        ax5.plot(t, series + i*4, color=colors_ex[i], linewidth=2, alpha=0.8, label=name)
    
    ax5.set_xlabel('Time')
    ax5.set_ylabel('Value (offset for clarity)')
    ax5.set_title('E. Sample DGP Time Series Examples', fontweight='bold', fontfamily=PUBLICATION_FONT)
    ...
    colors_perf = [PASTEL['excellent'] if x > 0.95 else PASTEL['good'] if x > 0.9 else PASTEL['moderate'] for x in auroc_by_type]
    
    bars6 = ax6.bar(dgp_types, auroc_by_type, color=colors_perf, alpha=0.8, edgecolor='gray')
    ax6.set_ylabel('AUROC Score')
    ax6.set_title('F. Validation Performance by DGP Type', fontweight='bold', fontfamily=PUBLICATION_FONT)
    ax6.set_ylim(0.85, 1.0)
    ax6.grid(axis='y', alpha=0.3)
    
    for bar, score in zip(bars6, auroc_by_type):
        height = bar.get_height()
        ax6.annotate(f'{score:.2f}', xy=(bar.get_x() + bar.get_width()/2, height),
                    xytext=(0, 3), textcoords='offset points',
                    ha='center', va='bottom', fontweight='bold')
    ...
    ax7.plot(cv_folds, cv_scores, 'o-', color=PASTEL['primary'], linewidth=3, markersize=8, alpha=0.8)
    ...
    ax7.set_title('G. 10-Fold Cross-Validation Results', fontweight='bold', fontfamily=PUBLICATION_FONT)
    ax7.legend()
    ax7.grid(alpha=0.3)
    ax7.set_ylim(0.94, 1.0)
    
    fig.suptitle('Synthetic DGP Atlas: 500 Data-Generating Processes for Calibration', 
                fontsize=18, fontweight='bold', fontfamily=PUBLICATION_FONT)
...
def create_all_scientific_plots(
    ...
    try:
        from causal_audit.plotting.diagnostic_dashboard import plot_comprehensive_diagnostic_dashboard
        plot_paths['diagnostic_dashboard'] = plot_comprehensive_diagnostic_dashboard(
            audit_evidence, risk_profile, output_path=output_dir / 'diagnostic_dashboard.png'
        )
    except ImportError:
    ...
    spec1 = importlib.util.spec_from_file_location("dash_enh", "causal_audit/plotting/dataset_dashboards_enhanced.py")
    dash_enh = importlib.util.module_from_spec(spec1)
    spec1.loader.exec_module(dash_enh)
    plot_synthetic_atlas_dashboard_enhanced = dash_enh.plot_synthetic_atlas_dashboard_enhanced
    
    spec2 = importlib.util.spec_from_file_location("dash_all", "causal_audit/plotting/dataset_dashboards_all_enhanced.py")
    ...
    spec3 = importlib.util.spec_from_file_location("dgp_atlas", "causal_audit/plotting/dgp_atlas_detailed.py")
