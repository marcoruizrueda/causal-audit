...
def plot_synthetic_atlas_walkthrough(output_path: Optional[str] = None) -> str:
    ...
    ax1.axvline(dates[250], color='red', linestyle='--', alpha=0.7, linewidth=2, label='Regime Switch')
    ...
    ax2 = axes[0, 1]
    risks = ['Nonstationarity', 'Irregularity', 'Persistence', 'Nonlinearity', 'Confounding']
    risk_vals = [0.92, 0.15, 0.35, 0.25, 0.20]
    colors = [PASTEL['poor'] if r > 0.7 else PASTEL['moderate'] if r > 0.3 else PASTEL['excellent'] for r in risk_vals]
    ...
    fig.suptitle('Real-World Walkthrough: Synthetic Atlas (500 DGPs)', 
...
def plot_timegraph_walkthrough(output_path: Optional[str] = None) -> str:
    ...
    ax1.grid(alpha=0.3)
    ax1.annotate('Strong Trend', xy=(dates[n//2], x[n//2]), xytext=(dates[n//2], x[n//2]+2),
    ...
    fig.suptitle('Real-World Walkthrough: TimeGraph Benchmark (18 Categories)', 
                fontsize=16, fontweight='bold', fontfamily=PUBLICATION_FONT)
    plt.tight_layout()
...
def plot_causaltime_walkthrough(output_path: Optional[str] = None) -> str:
    ...
    ax1.set_ylabel('Value')
    ax1.grid(alpha=0.3)
    ax1.annotate('Nonlinear Pattern', xy=(dates[n//2], x[n//2]), xytext=(dates[n//2], x[n//2]+1.5),
                arrowprops=dict(arrowstyle='->', color='red'), fontsize=9, color='red')
    ...
    fig.suptitle('Real-World Walkthrough: CausalTime Benchmark (3 Scenarios)', 
                fontsize=16, fontweight='bold', fontfamily=PUBLICATION_FONT)
    plt.tight_layout()
...
def plot_fluxnet_walkthrough(output_path: Optional[str] = None) -> str:
    ...
    missing_idx = gaps[0]
    ax1.annotate('Missing Data', xy=(dates[missing_idx], x_gapped[missing_idx-1]), 
    ...
    fig.suptitle('Real-World Walkthrough: FLUXNET-CH4 Dataset (5 Wetland Sites)', 
                fontsize=16, fontweight='bold', fontfamily=PUBLICATION_FONT)
    plt.tight_layout()
